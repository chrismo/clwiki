...
class ClWikiConfiguration
  ...
  def wait_on_threads
    ...
    @threads.each do |thread| thread.join end if !@threads.nil?
  end
  ...
  def initialize
    @cgifn = 'clwikicgi.rb'
    @url_prefix = '/'
    @indexPort = ClWikiIndexer.defaultPort
  ...
  def ClWikiConfiguration.scan_conf_lines(confLines, tag)
    confLines.grep(/^#{tag}/).to_s.scan(/^#{tag} (.*)/)
  end

  def ClWikiConfiguration.set_value(item_name, conf_lines)
    value = ClWikiConfiguration.scan_conf_lines(conf_lines, item_name).to_s
    $wikiConf.send(item_name + '=', value) if !value.empty?
  end

  def ClWikiConfiguration.load_xml(fileName=$defaultConfFile)
    if !$wikiConf
      ...
      # the file
      ClWikiConfiguration.set_value('wikiPath', confLines)
      ClWikiConfiguration.set_value('useIndex', confLines)
      ClWikiConfiguration.set_value('useIndexForPageExists', confLines)
      ClWikiConfiguration.set_value('editable', confLines)
      ClWikiConfiguration.set_value('indexPort', confLines)
      ClWikiConfiguration.set_value('cssHref', confLines)
      ClWikiConfiguration.set_value('template', confLines)
      ClWikiConfiguration.set_value('recentChangesName', confLines)
      ClWikiConfiguration.set_value('publishTag', confLines)
      ClWikiConfiguration.set_value('showSourceLink', confLines)
      ClWikiConfiguration.set_value('cgifn_from_rss', confLines)

      ClWikiConfiguration.set_value('enable_cvs', confLines)
      ClWikiConfiguration.set_value('cvs_log', confLines)
      
      ClWikiConfiguration.set_value('edit_rows', confLines)
      ClWikiConfiguration.set_value('edit_cols', confLines)
      
      ClWikiConfiguration.set_value('access_log_index', confLines)
      
      ClWikiConfiguration.set_value('index_log_fn', confLines)
